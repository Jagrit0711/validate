...
export function Footer() {
  return (
    <footer className="relative bg-[#050505] pt-24 pb-8 border-t border-white/10 overflow-hidden mt-auto">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between gap-16 md:gap-8 mb-20">
        ...
        <div className="md:w-1/2 flex flex-col">
          ...
          <div className="flex items-center gap-3 mb-8">
            <img src="https://raw.githubusercontent.com/Jagrit0711/zuup-main/bc25cc6dafa9026827ffffa84f5d6740d86950ab/public/lovable-uploads/b44b8051-6117-4b37-999d-014c4c33dd13.png" alt="Zuup" className="h-6 w-6 object-contain" />
            <span className="font-semibold tracking-tight text-white/90 text-2xl mt-1">Validate</span>
          </div>

          <h3 className="font-serif italic text-2xl sm:text-3xl text-white mb-2 font-light">
            Because trust needs proof.
          </h3>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 border-t border-white/10 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs font-mono text-white/50 tracking-widest">
        ...
        <span>Made with <span className="text-red-500">❤</span> by teens</span>
      </div>
    </footer>
  );
}
...
function SocialIcon({ href, icon }: { href: string; icon: React.ReactNode }) {
  return (
    <a 
      ...
      target="_blank" 
      rel="noreferrer"
      className="w-10 h-10 rounded-full border border-white/20 flex items-center justify-center text-white/70 hover:text-white hover:border-white/50 transition-all hover:scale-105"
    >
      {icon}
    </a>
  );
...
function FooterCol({ title, links }: { title: string; links: Array<{ href?: string; to?: string; label: string }> }) {
  return (
    <div>
      <p className="font-semibold tracking-tight text-xl text-white mb-6">{title}</p>
